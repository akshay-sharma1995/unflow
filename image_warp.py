...
# 	flow_floor = torch.floor(flow_flat).type(torch.IntTensor)
# 	bilinear_weights = flow_flat - torch.floor(flow_flat)

# 	pos_x = torch.arange(width).repeat(height*num_batch).type(torch.cuda.IntTensor).device(DEVICE)
...
# 	base = torch.reshape(base_grid, (-1,), dtype = torch.IntTensor, device = DEVICE)
